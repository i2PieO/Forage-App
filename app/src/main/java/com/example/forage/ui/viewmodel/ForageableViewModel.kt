...
class ForageableViewModel(
    ...
    val forageable: LiveData<List<Forageable>> = forageableDao.getAllForageables().asLiveData()
    ...
    fun getForageable(id: Long): LiveData<Forageable> =
        forageableDao.getForagebleById(id).asLiveData()
    ...
    fun deleteForageable(forageable: Forageable) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            forageableDao.deleteForageble(forageable)
        }
    }
